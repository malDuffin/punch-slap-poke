...
function classifyRps(lm: NormalizedLandmark[]): {
  ...
  // Scissors / peace / V: index + middle out, ring + pinky curled
  if (index && middle && !ring && !pinky) {
    return { gesture: "peace", mode: "poke", confidence: 0.92, label: "scissors" };
  }
  ...
  if (index && middle) {
    return { gesture: "peace", mode: "poke", confidence: 0.7, label: "scissors" };
  }
  if (extCount === 0) {
...
export class HandCameraTracker {
  ...
  private clickState = new Map<"L" | "R", { wasClosed: boolean; lastDist: number; cdUntil: number }>();
  ...
  /**
   * Finger click / snap: thumb tip + middle tip close then open quickly.
   * Also accepts thumb+index pinch-click.
   */
  private detectFingerClick(
    ...
    const dMid = dist(thumb, middle) / palm;
    const dIdx = index ? dist(thumb, index) / palm : 99;
    // Use the tighter of middle-snap or index-pinch
    ...
    const CLOSED = 0.55; // relative to palm
    const OPEN = 0.95;
    let clicked = false;
    if (!st.wasClosed && d < CLOSED) {
      st.wasClosed = true;
    } else if (st.wasClosed && d > OPEN) {
      // Released after pinch → click
      clicked = true;
      st.wasClosed = false;
      st.cdUntil = nowMs + 420;
    } else if (st.wasClosed && d > CLOSED * 1.15 && st.lastDist < CLOSED && d - st.lastDist > 0.12) {
      // Fast snap open
      clicked = true;
      st.wasClosed = false;
      st.cdUntil = nowMs + 420;
    }
    // timeout closed state if held too long without open
    if (st.wasClosed && d > OPEN * 1.2) st.wasClosed = false;
    st.lastDist = d;
    return clicked;
  }
